...
import Data.List
import System.IO
import Control.Exception (bracket, evaluate, try, IOException)
...
guardarParqueadero parqueadero =
    bracket (openFile "parqueadero.txt" WriteMode) hClose $ \h -> do
        hPutStr h (unlines (map show parqueadero)) -- Usamos show en vez de mostrarVehiculo
...
cargarParqueadero = do
    resultado <- try $ bracket (openFile "parqueadero.txt" ReadMode) hClose $ \h -> do
        contenido <- hGetContents h
        -- Forzar la evaluación completa del contenido antes de cerrar el handle
        contenidoEvaluado <- evaluate (length contenido)
        let lineas = lines contenido
        return (map read lineas)
    case resultado of
        Left e -> do
            let err = show (e :: IOException)
            putStrLn $ "No se pudo cargar el parqueadero: " ++ err
            return []
        Right parqueadero -> return parqueadero
...
-- Función para listar los vehículos cargados desde el archivo
listarVehiculos :: IO ()
listarVehiculos = do
